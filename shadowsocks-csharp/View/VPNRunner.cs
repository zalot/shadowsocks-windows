...
{
    public partial class VPNRunner : Form
    {
        ...
        public VPNRunner(ShadowsocksController controller)
        {
        ...
        private bool SaveOldSelectedServer()
        {
            try
            {
                if (_lastSelectedIndex == -1 || _lastSelectedIndex >= _modifiedConfiguration.servers.Count)
                {
                    return true;
                }
                ...
                _modifiedConfiguration.servers[_lastSelectedIndex] = server;
        ...
        private void ConfigForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                controller.SaveServers(_modifiedConfiguration.servers, _modifiedConfiguration.localPort);
                controller.SelectServerIndex(_modifiedConfiguration.servers.IndexOf(server));
            }
        ...
        private void ServersListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                ServersListBox.Items[_lastSelectedIndex] = _modifiedConfiguration.servers[_lastSelectedIndex].FriendlyName();
            }
            UpdateMoveUpAndDownButton();
            LoadSelectedServer();
        ...
        private void DuplicateButton_Click( object sender, EventArgs e )
        {
            ...
            Server currServer = _modifiedConfiguration.servers[_lastSelectedIndex];
            var currIndex = _modifiedConfiguration.servers.IndexOf( currServer );
            _modifiedConfiguration.servers.Insert(currIndex + 1, currServer);
            LoadConfiguration(_modifiedConfiguration);
        ...
        {
            _lastSelectedIndex = ServersListBox.SelectedIndex;
            if (_lastSelectedIndex >= 0 && _lastSelectedIndex < _modifiedConfiguration.servers.Count)
            {
                _modifiedConfiguration.servers.RemoveAt(_lastSelectedIndex);
            }
            if (_lastSelectedIndex >= _modifiedConfiguration.servers.Count)
            {
                // can be -1
                _lastSelectedIndex = _modifiedConfiguration.servers.Count - 1;
            }
            ServersListBox.SelectedIndex = _lastSelectedIndex;
            LoadConfiguration(_modifiedConfiguration);
            ServersListBox.SelectedIndex = _lastSelectedIndex;
        ...
        private void MoveConfigItem(int step)
        {
            ...
            Server server = _modifiedConfiguration.servers[index];
            object item = ServersListBox.Items[index];

            _modifiedConfiguration.servers.Remove(server);
            _modifiedConfiguration.servers.Insert(index + step, server);
